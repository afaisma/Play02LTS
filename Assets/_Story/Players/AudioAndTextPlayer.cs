...
public class AudioAndTextPlayer : MonoBehaviour
{
    ...
    void PreparePlayVoiceSettings()
    {
        ...
        {
            // Human narration highlights too, WHEN aligned timings exist for the bare chunk
            // ({chunk}_timings.json, produced by ASR forced alignment of the recording).
            // Books without them degrade exactly as before: the timings fetch 404s, the page
            // renders as one plain token, and the Count>1 guard in UpdateHighlightedText
            // keeps that single token from being painted as one giant highlight.
            showHighlight = true;
        ...
        float endTime = -1)
    {
        ...
        // If not generated by TTS or if we want a plain chunk. Human recordings pair with
        // {chunk}_timings.json (ASR-aligned); missing timings fail soft (warned fetch, no highlight).
        if (!audioNameGenerated)
        {
            audioURL = AppendContentRev($"{chunkname}.mp3");
            jsonTimingsURL = AppendContentRev($"{chunkname}_timings.json");
        }
    ...
    private void UpdateHighlightedText(float currentAudioTime, bool bHilight = true)
    {
        ...
        {
            // Count > 1: a single-token list is the whole-page fallback (no real timings) —
            // highlighting it would paint the entire page as one word.
            bool isHighlighted = (showHighlight && bHilight && currentWordTimings.Count > 1
                                  && i == currentWordIndex && !IsWordPunctuation(i));
            if (isHighlighted)
            {
